...
const total = db.prepare('SELECT COUNT(*) c FROM memory_turns').get().c
console.log(`total turns: ${total}`)

const rs = db.prepare('SELECT * FROM memory_runtime_state').all()
console.log(`runtime states: ${rs.length}`)
for (const r of rs) {
  console.log(`  session=${r.session_key} ctx=${r.runtime_context_id} epoch=${r.runtime_epoch} updated=${r.updated_at}`)
}

const sessions = db.prepare('SELECT session_key, COUNT(*) c FROM memory_turns GROUP BY session_key').all()
