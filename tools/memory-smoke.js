...
try {
  ...
  const currentRuntimeTurn = (await store.listRecentTurns({ sessionKey, limit: 5 }))
    .find((t) => /Current runtime only detail/.test(t.content))
  ...
  assert.equal(
    legacyContext.recalledTurns.some((t) => /Legacy runtime detail/.test(t.content)),
    true,
  )
  ...
  assert.equal(
    filteredContext.recalledTurns.some((t) => /Current runtime only detail/.test(t.content)),
    false,
  )
  ...
  assert.equal(
    postCompactionContext.recalledTurns.some((t) => /Current runtime only detail/.test(t.content)),
    true,
  )
