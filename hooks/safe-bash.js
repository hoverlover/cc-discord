 ...
 * - ALLOW_BASH_RUN_IN_BACKGROUND=true|false (default: false)
...
function normalizeMode(value) {
  const v = String(value || 'block').toLowerCase().trim()
  return v === 'allow' ? 'allow' : 'block'
}

function hasStandaloneBackgroundOperator(command) {
...
function truncateCommand(command, max = 220) {
  const clean = String(command || '').replace(/[`\r\n\t]+/g, ' ').trim()
  if (clean.length <= max) return clean
  return `${clean.slice(0, max - 1)}…`
}

async function notifyDiscord({ blocked, reasons, command }) {
...
try {
  ...
  const allowRunInBackground = asBool(process.env.ALLOW_BASH_RUN_IN_BACKGROUND, false)
